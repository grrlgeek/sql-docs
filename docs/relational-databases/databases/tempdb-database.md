...
- SQL Setup adds multiple `tempdb` data files during a new instance installation. Review the recommendedations and configure your `tempdb` in the **Database Engine Configuration** page of SQL Setup, or use the command-line parameter `/SQLTEMPDBFILECOUNT`. By default, SQL Setup adds as many `tempdb` data files as the number of logical processors or eight, whichever is lower.
